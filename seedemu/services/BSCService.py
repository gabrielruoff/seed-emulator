...
for i in ${variable//,/ }
do
    geth --exec "admin.addPeer(\\"$i\\")" attach http://localhost:8545
done
'''
...
class BSCServer(Server):
    ...
    def install(self, node: Node, eth: 'DeLTService', allBootnode: bool):
        ...
        common_args = '{} --identity="NODE_{}" --networkid=1000 --verbosity=9 --allow-insecure-unlock --nat=extip:$(/tmp/bsc-ipdiscover) --http --http.port 8545 --http.addr localhost'.format(datadir_option, self.__id)
